...
for product in cockroach cockroach-sql; do
  ...
  for platform in darwin-11.0-aarch64; do
    ...
    # TODO: the following line can be fixed by adding an extra space in the generated checksum files to meet the widely adopted 2-space format. Beta 5 will have it.
    grep "  " "$unsigned_file.sha256sum" || sed -i '' -e "s/ /  /g" "$unsigned_file.sha256sum"
